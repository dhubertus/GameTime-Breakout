...
class Board {
  ...
  blockTopBottomCollision() {
    for (let i = 0; i < this.block.length; i++) {
      if (
        ...
        this.ball.y <= this.block[i].y + this.block[i].height)
        {
  ...
  blockSideCollision() {
    for (let i = 0; i < this.block.length; i++) {
      if (
        ...
        this.ball.x < this.block[i].y + this.block[i].height &&
        this.ball.x > this.block[i].y - 10) {
          this.block.splice(i, 1)
          this.ball.vx = -this.ball.vx
          this.changeScoreAndLevel()
        }
  ...
  gameDraw(timer) {
    ...
    // this.blockCollision()
    // this.blockTopBottomCollision()
    this.blockSideCollision()
    if (this.level == 2) {
